...
function StartupListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<StartupInterface[]>(
    ...
    () =>
      getStartups({
        relations: ['user', 'airdrop.count', 'reward.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('airdrop', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>airdrop</Th>}
                  {hasAccess('reward', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>reward</Th>}
                  <Th>Actions</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('airdrop', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.airdrop}</Td>
                    )}
                    {hasAccess('reward', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.reward}</Td>
                    )}
                    <Td>
